import { formatDisplayValue, formatDateTime, capitalizeWords, formatComparative } from "./utils.js";
...
export function updateComparative(data) {
  document.querySelector(".value-yesterday-temperature").textContent = formatComparative(data.comparative.yesterday?.temperature, data.reference.temperature, "temp");
  document.querySelector(".value-yesterday-efficiency").textContent = formatComparative(data.comparative.yesterday?.efficiency, data.reference.efficiency, "eff");

  document.querySelector(".value-last-week-temperature").textContent = formatComparative(data.comparative.lastWeek?.temperature, data.reference.temperature, "temp");
  document.querySelector(".value-last-week-efficiency").textContent = formatComparative(data.comparative.lastWeek?.efficiency, data.reference.efficiency, "eff");

  document.querySelector(".value-last-month-temperature").textContent = formatComparative(data.comparative.lastMonth?.temperature, data.reference.temperature, "temp");
  document.querySelector(".value-last-month-efficiency").textContent = formatComparative(data.comparative.lastMonth?.efficiency, data.reference.efficiency, "eff");
}
