...
async function exportPDF(req, res) {
    try {
        ...
        const [history, stats, latest] = await Promise.all([
            metricService_1.metricService.getHistoryByRange(range),
            metricService_1.metricService.getStatsByRange(range),
            metricService_1.metricService.getLatest()
        ]);
...
function compileHTMLTemplate(data) {
    const template = `
  <html lang="pt-BR">
  <head>
    <meta charset="UTF-8" />
